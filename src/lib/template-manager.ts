...
export class TemplateManager {
  ...
  public prepareTemplateData(templateId: string): TemplateLoadResult {
    try {
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Unknown error preparing template data'
      };
    }
  }
    ...
    try {
      ...
      if (Array.isArray(parsedData) && parsedData.length > 0 && parsedData[0].id && parsedData[0].data) {
        ...
        if (success) {
          ...
          console.error('❌ TemplateManager: Failed to load multi-page template data');
        }
      ...
      if (success) {
        ...
        console.error('❌ TemplateManager: Failed to load single-page template data');
      }
