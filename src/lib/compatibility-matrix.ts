...
export class CompatibilityMatrix {
  ...
  testCompatibility(templateId: string, themeId: string): CompatibilityResult {
    ...
    if (specificRule && !specificRule.compatible) {
      ...
      result.score = 0
      result.issues.push(specificRule.reason || 'Explicitly marked as incompatible')
      return result
    }
    ...
    if (!theme.compatibleTemplates.includes('*') && 
    ...
    if (missingFeatures.length > 0) {
      ...
      result.warnings.push(`Missing theme features: ${missingFeatures.join(', ')}`)
    }

    // Category compatibility scoring
    if (template.category === theme.category) {
      result.score += 10
      result.recommendations.push('Template and theme categories match perfectly')
    } else {
      result.score -= 5
    ...
    if (template.isPremium && !theme.isPremium) {
      result.score -= 10
      result.warnings.push('Premium template with free theme may have limited features')
    }
  ...
  private getRule(templateId: string, themeId: string): CompatibilityRule | null {
  ...
  getCompatibleTemplates(themeId: string): string[] {
    ...
    return allTemplates
      .filter(template => this.testCompatibility(template.id, themeId).compatible)
  ...
  getFullMatrix(): Record<string, Record<string, CompatibilityResult>> {
    ...
    templates.forEach(template => {
      ...
      themes.forEach(theme => {
        matrix[template.id][theme.id] = this.testCompatibility(template.id, theme.id)
      })
    })
  ...
  getBestThemes(templateId: string, limit: number = 5): Array<{themeId: string, score: number}> {
    const allThemes = this.themeRegistry.getAllThemes()
  ...
  getBestTemplates(themeId: string, limit: number = 5): Array<{templateId: string, score: number}> {
    const allTemplates = this.templateRegistry.getAllTemplates()
...
export function testCompatibility(templateId: string, themeId: string): CompatibilityResult {
...
export function getBestThemeRecommendations(templateId: string, limit?: number) {
  const matrix = CompatibilityMatrix.getInstance()
  return matrix.getBestThemes(templateId, limit)
}

export function getBestTemplateRecommendations(themeId: string, limit?: number) {
  const matrix = CompatibilityMatrix.getInstance()
