...
import { Crown, Zap, Check, X, Calendar, Shield, Loader2, ExternalLink, AlertTriangle, Sparkles, Gift, Building2, ChevronRight, Rocket, TrendingUp, Star, Award } from 'lucide-react'
        ...
        <div className="p-6">
          ...
          <div className="mx-auto">
                ...
                <div className="relative mb-6 overflow-hidden rounded-2xl bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 p-6 shadow-lg">
                  <div className="relative">
                    <div className="mb-4 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div className={`flex h-12 w-12 items-center justify-center rounded-lg bg-white/20 backdrop-blur-md shadow-md`}>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-2">
                            <h1 className="text-2xl font-bold text-white">{PLAN_FEATURES[data.plan].name} Plan</h1>
                            {data.subscription ? (
                              <span className={`rounded-full px-3 py-1 text-sm font-semibold ${data.subscription.status === 'ACTIVE' ? 'bg-emerald-500 text-white' : 'bg-red-500 text-white'}`}>
                                {data.subscription.status}
                              </span>
                            ) : (
                              <span className="rounded-full px-3 py-1 text-sm font-semibold bg-indigo-500 text-white">Free</span>
                            )}
                          </div>
                          <p className="text-sm text-purple-100">{PLAN_FEATURES[data.plan].description}</p>
                        </div>
                      </div>
                      ...
                      <div className="mb-4 grid grid-cols-1 gap-3 md:grid-cols-2">
                        ...
                        <div className="rounded-lg bg-white/10 p-4 backdrop-blur-md">
                          <div className="mb-1 flex items-center gap-2 text-white/80">
                            ...
                            <span className="text-xs font-medium">Next Billing</span>
                          </div>
                          <div className="text-sm font-bold text-white">
                    ...
                    <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                      <div className="rounded-lg bg-white p-4 shadow-sm">
                        <div className="mb-2 flex items-center justify-between">
                          <span className="text-xs font-medium text-gray-600">Catalogues Used</span>
                          <TrendingUp className="h-4 w-4 text-indigo-600" />
                        </div>
                        <div className="mb-1 flex items-baseline gap-2">
                          <span className="text-2xl font-bold text-gray-900">{usage?.catalogues ?? 0}</span>
                          <span className="text-xs text-gray-500">/ {planFeatures.maxCatalogues === -1 ? '∞' : planFeatures.maxCatalogues}</span>
                        </div>
                      ...
                      <div className="rounded-lg bg-white p-4 shadow-sm">
                        <div className="mb-2 flex items-center justify-between">
                          <span className="text-xs font-medium text-gray-600">Monthly Exports</span>
                          <Award className="h-4 w-4 text-emerald-600" />
                        </div>
                        <div className="mb-1 flex items-baseline gap-2">
                          <span className="text-2xl font-bold text-gray-900">{usage?.monthlyExports ?? 0}</span>
                          <span className="text-xs text-gray-500">/ {planFeatures.maxExportsPerMonth === -1 ? '∞' : planFeatures.maxExportsPerMonth}</span>
                        </div>
                        <div className="h-2 w-full overflow-hidden rounded-full bg-gray-200">
                          <div
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="text-white">
                      <h2 className="mb-1 text-2xl font-bold">Ready to Level Up?</h2>
                      <p className="mb-2 text-sm text-orange-100">
                        Upgrade to <strong>{nextPlanFeatures.name}</strong> and unlock more powerful features
                      </p>
                      <div className="flex flex-wrap gap-2">
                        {nextPlanFeatures.included.slice(0, 3).map((feature, idx) => (
                          <span key={idx} className="inline-flex items-center gap-1 rounded-full bg-white/20 px-2 py-0.5 text-xs font-medium backdrop-blur-md">
                  ...
                  <button
                    ...
                    disabled={isProcessing}
                    className="flex items-center gap-2 rounded-lg bg-white px-4 py-2 text-sm font-semibold text-orange-600 shadow-md transition-all hover:scale-102 disabled:opacity-50"
                  >
                    {isProcessing ? (
                      <>
            ...
            <div className="mb-6 rounded-2xl bg-white p-6 shadow-lg">
              <div className="mb-4 flex items-center justify-between">
                <div>
                  <h2 className="mb-1 text-2xl font-bold text-gray-900">Choose Your Plan</h2>
                  <p className="text-sm text-gray-600">Select the perfect plan for your creative needs</p>
                </div>
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
                {[SubscriptionPlan.FREE, SubscriptionPlan.STANDARD, SubscriptionPlan.PROFESSIONAL, SubscriptionPlan.BUSINESS].map((plan, idx) => {
                  ...
                  return (
                    <div
                      key={plan}
                      className={`group relative overflow-hidden rounded-2xl border-2 transition-all hover:shadow-2xl ${isCurrentPlan ? 'border-emerald-500 shadow-xl' : isRecommended ? 'border-purple-500 shadow-lg' : 'border-gray-200 hover:border-gray-300'
                        }`}
                    >
                      ...
                      <div className={`p-4 ${isRecommended || isCurrentPlan ? 'pt-10' : ''}`}>
                        ...
                        {isCurrentPlan ? (
                          ...
                          >
                            {isProcessing ? 'Processing...' : `Upgrade to ${features.name}`}
                          </button>
                        )}
                      </div>
                    </div>
            ...
            <div className="mb-6 rounded-2xl bg-gradient-to-br from-emerald-50 to-teal-50 p-6 shadow-md">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <h3 className="mb-1 text-xl font-bold text-gray-900">Have a Coupon Code?</h3>
                  <p className="mb-2 text-sm text-gray-600">Enter your code to unlock special discounts</p>
            ...
            <div className="rounded-2xl bg-white p-6 shadow-lg">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <h3 className="mb-2 text-xl font-bold text-gray-900">Secure & Transparent Billing</h3>
