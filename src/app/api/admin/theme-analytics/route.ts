...
async function calculateThemePopularityMetrics(timeRange: string) {
  ...
  const averageSelectionsPerTheme =
    totalThemes.length > 0 ? totalSelections / totalThemes.length : 0
  ...
  return {
    ...
    averageSelectionsPerTheme:
      Math.round(averageSelectionsPerTheme * 100) / 100,
...
async function calculateTrendingThemes() {
  ...
  const trends = currentUsage.map(current => {
    ...
    return {
      ...
      trend:
        growthRate > 10 ? 'rising' : growthRate < -10 ? 'falling' : 'stable',
    }
  })
...
async function calculateThemeAdoptionRates() {
  ...
  const adoptionRates = themeSelections.map(theme => ({
    ...
    adoptionRate:
      totalCatalogues > 0
        ? ((theme._count.id / totalCatalogues) * 100).toFixed(2)
        : '0.00',
  }))
  ...
  return {
    ...
    averageAdoptionRate:
      adoptionRates.length > 0
        ? (
            adoptionRates.reduce(
              (sum, theme) => sum + parseFloat(theme.adoptionRate),
              0
            ) / adoptionRates.length
          ).toFixed(2)
        : '0.00',
  }
}
