...
export async function GET(request: NextRequest) {
  try {
    ...
    const csvData = users.map(user => ({
      ...
      'Subscription Amount': user.subscriptions?.[0]?.amount
        ? Number(user.subscriptions[0].amount)
        : 0,
      'Subscription End Date':
        user.subscriptions?.[0]?.currentPeriodEnd?.toISOString() || 'N/A',
      'Subscription Created':
        user.subscriptions?.[0]?.createdAt?.toISOString() || 'N/A',
