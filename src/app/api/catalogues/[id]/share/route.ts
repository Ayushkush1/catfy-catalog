...
export async function POST(
    ...
    try {
        ...
        const catalogue = await prisma.catalogue.findFirst({
            ...
            include: { profile: { select: { email: true, fullName: true } }, invitations: true },
        })

        if (!catalogue) {
            return NextResponse.json({ error: 'Catalogue not found or you are not the owner' }, { status: 404 })
        }
