...
export async function GET(request: NextRequest) {
    try {
        ...
        const publishedCatalogues = catalogues.filter(c => c.status === 'PUBLISHED').length
        const draftCatalogues = catalogues.filter(c => c.status === 'DRAFT').length
        const archivedCatalogues = catalogues.filter(c => c.status === 'ARCHIVED').length

        const totalProducts = catalogues.reduce((sum, cat) => sum + cat._count.products, 0)
        const totalCategories = catalogues.reduce((sum, cat) => sum + cat._count.categories, 0)
        ...
        const themeCounts = themeUsage.reduce((acc, theme) => {
        ...
        const previousEventCounts = previousAnalytics.reduce((acc, event) => {
            acc[event.event] = (acc[event.event] || 0) + 1
            return acc
        }, {} as Record<string, number>)
        ...
        for (let i = 6; i >= 0; i--) {
            ...
            const dayEvents = analytics.filter(event =>
        ...
        const userTemplates = templates.filter(t => t.authorId === profile.id).length
