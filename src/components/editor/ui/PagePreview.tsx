...
export const PagePreview: React.FC<PagePreviewProps> = ({
  ...
  const generateThumbnail = async () => {
    ...
    try {
      ...
      const frameElement = editorRef.current.querySelector('[data-cy="frame"]') || 
      ...
      const canvas = await html2canvas(frameElement as HTMLElement, {
        ...
        ignoreElements: (element) => {
          ...
          return element.classList.contains('craft-layer') || 
                 ...
                 !!element.closest('.craft-layer, .craft-indicator, .craft-toolbar, .craft-sidebar');
        }
      });
      ...
      if (parsedData.ROOT && parsedData.ROOT.nodes && parsedData.ROOT.nodes.length > 0) {
