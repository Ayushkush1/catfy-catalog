...
import { templateRegistry, registerTemplates } from './registry/template-registry';
...
const existingTemplates = PrebuiltTemplates.filter(template => 
  !modularTemplates.some(modular => modular.id === template.id)
);
