...
export const LayersPanel: React.FC<LayersPanelProps> = ({ className }) => {
  ...
  const buildLayerTree = (): LayerNode[] => {
    ...
    Object.entries(nodes).forEach(([nodeId, node]) => {
      ...
      const layerState = layerStates.get(nodeId) || {
        isVisible: true,
        isLocked: false,
      }
  ...
  const updateLayerState = (nodeId: string, updates: Partial<LayerState>) => {
    setLayerStates(prev => {
      ...
      const currentState = newMap.get(nodeId) || {
        isVisible: true,
        isLocked: false,
      }
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    try {
      ...
      if (overNode) {
        ...
        const childrenCount = overNode.data.nodes
          ? overNode.data.nodes.length
          : 0
  ...
  const renderLayerNodeContent = (
    ...
    return (
      <div key={node.id} className="select-none">
        ...
        <div
          className={`
            group relative flex cursor-pointer items-center px-2 py-1.5 transition-all duration-200
            ${
              isSelected
                ? 'border-l-2 border-blue-500 bg-blue-50 shadow-sm'
        ...
        >
          ...
          <div
            {...listeners}
            className={`
              mr-1.5 flex h-3 w-3 cursor-grab items-center justify-center opacity-0
              transition-opacity active:cursor-grabbing group-hover:opacity-100
          ...
          <div className="mr-1.5 flex h-4 w-4 items-center justify-center">
            {hasChildren ? (
              <button
                ...
                className="rounded-sm p-0.5 transition-colors hover:bg-gray-200"
              >
                {isExpanded ? (
          ...
          {/* Node Icon */}
          <div
            className={`mr-2 text-gray-600 ${layerState.isLocked ? 'text-gray-400' : ''}`}
          >
          ...
          {/* Status Indicators */}
          <div className="mr-1.5 flex items-center space-x-0.5">
            {layerState.isLocked && (
          ...
          <div className="flex items-center space-x-0.5 opacity-0 transition-opacity group-hover:opacity-100">
            ...
            >
              <Trash2
                className={`h-3 w-3 ${layerState.isLocked ? 'text-gray-300' : 'text-red-500'}`}
              />
            </button>
          </div>
        </div>
    ...
    >
      ...
      <div className="border-b border-gray-200 bg-gray-50 px-3 py-2">
        <h3 className="flex items-center text-xs font-semibold text-gray-800">
          <Layers className="mr-1.5 h-3 w-3 text-gray-600" />
          Layers
        </h3>
      </div>
          ...
          >
            ...
            <DragOverlay>
              {activeId && draggedItem ? (
                <div className="flex items-center space-x-1.5 rounded-md border border-gray-300 bg-white px-2 py-1.5 shadow-lg">
                  <div className="text-gray-600">
                    {React.cloneElement(getNodeIcon(draggedItem.type), {
                      className: 'w-3 h-3',
                    })}
                  </div>
