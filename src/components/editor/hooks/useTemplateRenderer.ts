...
export const useTemplateRenderer = (options: UseTemplateRendererOptions = {}) => {
      ...
      const loadSuccess = templateManager.loadTemplateIntoEditor(
        ...
        (data: string) => {
          try {
            ...
            console.log('✅ Single-page template data loaded into editor successfully');
            return true;
          } catch (error) {
            console.error('❌ Failed to deserialize single-page template data:', error);
            return false;
          }
        },
        multiPageHook ? (pages: any[]) => {
          try {
            ...
            console.error('❌ Failed to load multi-page template data:', error);
            return false;
          }
        } : undefined
      );
  ...
  const loadFromStorage = useCallback(async (): Promise<TemplateLoadResult | null> => {
    ...
    try {
      ...
      const loadSuccess = templateManager.loadTemplateIntoEditor(
        ...
        (data: string) => {
          try {
            ...
            console.error('❌ Failed to deserialize stored template data:', error);
            return false;
          }
        },
        multiPageHook ? (pages: any[]) => {
          try {
            ...
            console.error('❌ Failed to load multi-page template data from storage:', error);
            return false;
          }
        } : undefined
      );
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error loading stored template';
      setState(prev => ({ ...prev, isLoading: false, error: errorMessage }));
      onTemplateError?.(errorMessage);
      return { success: false, error: errorMessage };
    }
  ...
  return {
    ...
    getStoredTemplateId: () => templateManager.getStoredSelection()?.templateId || null
  };
};
