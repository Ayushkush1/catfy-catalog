...
export const useExportImport = () => {
  ...
  const exportAsPNG = useCallback(
    async (
      ...
      const { quality = 1, scale = 2, filename = 'page' } = options
  ...
  const exportAsPDF = useCallback(
    async (
      ...
      const { scale = 2, filename = 'document' } = options
  ...
  const exportAsJSON = useCallback(
    (pages: Page[], options: Partial<ExportOptions> = {}): void => {
      const { filename = 'pages-data', multiPage = true } = options
...
`
  ...
  const importFromJSON = useCallback(
    async (file: File, options: ImportOptions = {}): Promise<Page[]> => {
        ...
        reader.onload = e => {
          try {
            ...
            if (importData.pages && Array.isArray(importData.pages)) {
              console.log(
                '📚 Multi-page format detected, pages:',
                importData.pages.length
              )
              ...
              console.log(
                '🏗️ ROOT node type:',
                importData.ROOT?.type?.resolvedName
              )
              console.log(
                '📦 ROOT node children:',
                importData.ROOT?.nodes?.length || 0
              )
              ...
              if (validateData && importData.ROOT) {
                ...
                if (typeof importData.ROOT !== 'object') {
                  throw new Error(
                    'Invalid CraftJS data: ROOT must be an object'
                  )
                }
              }
              ...
              const pageName =
                file.name.replace('.json', '') || 'Imported Template'
              ...
              console.log('📦 Created single page:', {
                id: pageId,
                name: pageName,
                dataLength: JSON.stringify(importData).length,
              })
            } else {
              console.error('❌ Invalid JSON format - missing ROOT or pages')
              console.error('📊 Available keys:', Object.keys(importData))
              throw new Error(
                'Invalid JSON format: Expected either pages array or CraftJS data structure'
              )
            }
            ...
            console.log(
              '🚀 Import completed successfully, returning pages:',
              pages.length
            )
            resolve(pages)
          } catch (error) {
  ...
  const importPageData = useCallback(
    async (file: File, options: ImportOptions = {}): Promise<string> => {
      const { validateData = true } = options
      ...
      return new Promise((resolve, reject) => {
        ...
        reader.onload = e => {
          try {
            ...
            reject(
              new Error(
                `Failed to import page data: ${error instanceof Error ? error.message : 'Unknown error'}`
              )
            )
          }
        }
  ...
  const triggerImport = useCallback(
    (onImport: (file: File) => void, accept: string = '.json') => {
    ...
    (data: string) => {
      try {
        ...
        try {
          ...
          throw new Error(
            `Deserialization failed: ${(deserializeError as Error).message}`
          )
        }
