...
export const useMultiPage = (options: UseMultiPageOptions = {}) => {
  ...
  const addPage = useCallback(
    (name?: string, templateData?: string) => {
  ...
  const addPages = useCallback(
    (newPages: Page[], switchToFirstImported: boolean = true) => {
  ...
  const loadPages = useCallback(
    (newPages: Page[], initialPageId?: string) => {
      ...
      if (targetPage) {
        try {
          ...
          console.log(
            '📊 Page data preview:',
            targetPage.data.substring(0, 200) + '...'
          )
          ...
          setTimeout(() => {
            ...
            console.log(
              ...
              Object.keys(currentNodes).length
            )
            console.log(
              '🏗️ Node types:',
              Object.values(currentNodes).map(
                n => n.data.displayName || n.data.type
              )
            )
          }, 100)
        } catch (error) {
