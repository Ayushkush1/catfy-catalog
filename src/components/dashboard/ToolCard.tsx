...
export function ToolCard({
  ...
  return (
    <Card
      className={`group relative flex flex-col overflow-hidden rounded-2xl border-0 p-2 shadow-md transition-all duration-300 ${
        tool.isActive
    ...
    >
      ...
      <CardHeader className="relative pb-3">
        <div className="mb-2 flex items-center gap-3">
          <div
            className={`flex h-12 w-12 items-center justify-center rounded-xl ${tool.gradient} shadow-md transition-transform duration-300 group-hover:scale-110`}
          >
      ...
      <CardContent className="relative flex flex-1 flex-col justify-between space-y-4 pt-0">
        {/* Features */}
        ...
        {tool.stats.length > 0 && (
          <div
            className={`flex items-center gap-3 rounded-lg ${tool.isComingSoon ? 'bg-gradient-to-r from-amber-50 to-orange-50' : `bg-gradient-to-r from-blue-50 to-purple-50`} p-3`}
          >
            {tool.isComingSoon ? (
              ...
              tool.stats.map((stat, index) => (
                <div key={index} className="flex-1 text-center">
                  ...
                  {index < tool.stats.length - 1 && (
                    <div
                      className="absolute h-10 w-px bg-gray-300"
                      style={{
                        left: `${((index + 1) / tool.stats.length) * 100}%`,
                      }}
                    ></div>
                  )}
                </div>
